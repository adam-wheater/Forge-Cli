function Detect-TestStyle {
    ...
    # ⚡ Bolt Optimization: Consolidate multiple regex patterns using alternation (|)
    # to scan the entire string once per framework instead of 3-5 times.
    ...
    # ⚡ Bolt Optimization: Consolidate regex patterns to single scan passes.
    # Note: Using `new\s+Mock<` alongside `Mock<` in alternation counts them as one if overlapping,
    # resolving double-counting bugs while speeding up the detection.
    ...
    # ⚡ Bolt Optimization: Consolidate regex patterns.
    ...
    # ⚡ Bolt Optimization: Use RegexOptions.IgnoreCase for faster scanning and robustness.
    $ignoreCase = [System.Text.RegularExpressions.RegexOptions]::IgnoreCase
    $arrangeMatches = ([regex]::Matches($allText, $arrangePattern, $ignoreCase)).Count
    $actMatches = ([regex]::Matches($allText, $actPattern, $ignoreCase)).Count
    $assertMatches = ([regex]::Matches($allText, $assertPattern, $ignoreCase)).Count
...
function Get-TestStylePrompt {
    ...
    # ⚡ Bolt Optimization: Replace += array concatenation with a List to avoid O(N^2) reallocation overhead
