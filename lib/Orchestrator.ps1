...
function Invoke-RunTests {
    ...
    try {
        ...
        $result = @{
            # Optimization: Use Generic List[T] instead of array concatenation (+=) for O(1) appends instead of O(N^2)
        ...
        foreach ($m in $passMatches) {
            [void]$result.Passed.Add($m.Groups[1].Value)
        }
...
function Invoke-ReadTestOutput {
    ...
    try {
        ...
        return ($results.ToArray() | ConvertTo-Json -Depth 5 -Compress)
    } catch {
    ...
    try {
        ...
        foreach ($pkg in $packages) {
            ...
            foreach ($cls in $classes) {
                ...
                if ($cls.lines -and $cls.lines.line) {
                    foreach ($line in $cls.lines.line) {
                        if ([int]$line.hits -eq 0) {
                            [void]$uncovered.Add([int]$line.number)
                        }
                    }
                }
                ...
                if ($uncovered.Count -gt 0) {
                    ...
                    for ($i = 1; $i -lt $uncovered.Count; $i++) {
                        if ($uncovered[$i] -eq $end + 1) {
                            ...
                            [void]$ranges.Add($(if ($start -eq $end) { "$start" } else { "$start-$end" }))
                            $start = $uncovered[$i]
                            $end = $uncovered[$i]
                        }
                    }
                ...
                [void]$lines.Add("CLASS: $className — ${lineRate}% covered${uncoveredStr}")
            }
        }
...
function Invoke-ListTests {
    ...
    try {
        ...
        foreach ($name in $testNames) {
            ...
            [void]$grouped[$classShort].Add($methodName)
        }

        $lines = [System.Collections.Generic.List[string]]::new()
        foreach ($cls in $grouped.Keys | Sort-Object) {
            $methods = $grouped[$cls] -join ", "
            [void]$lines.Add("${cls}: $methods")
        }
