...
function Invoke-RunTests {
    ...
    try {
        ...
        # Convert lists to arrays for JSON serialization
        $result.Passed = $result.Passed.ToArray()
        ...
        foreach ($pkg in $packages) {
            ...
            foreach ($cls in $classes) {
                ...
                if ($uncovered.Count -gt 0) {
                    $uncoveredArray = $uncovered.ToArray() | Sort-Object
                    $start = $uncoveredArray[0]
                    $end = $uncoveredArray[0]
                    for ($i = 1; $i -lt $uncoveredArray.Count; $i++) {
                        if ($uncoveredArray[$i] -eq $end + 1) {
                            $end = $uncoveredArray[$i]
                        } else {
                            $ranges.Add($(if ($start -eq $end) { "$start" } else { "$start-$end" }))
                            $start = $uncoveredArray[$i]
                            $end = $uncoveredArray[$i]
                        }
                    }
                    $ranges.Add($(if ($start -eq $end) { "$start" } else { "$start-$end" }))
