...
function Score-File {
    ...
    $score = 0
    # Optimisation: Use if/elseif/EndsWith/IndexOf for performance instead of regex

    # Extensions (using EndsWith)
    ...
    if ($Path.IndexOf('Service', [System.StringComparison]::OrdinalIgnoreCase) -ge 0 -or
        ...
        $Path.IndexOf('Repo', [System.StringComparison]::OrdinalIgnoreCase) -ge 0) { # Repo covers Repository
        $score += 15
    }
