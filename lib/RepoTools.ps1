...
function Score-File {
    ...
    # Optimisation: Use native .NET string methods instead of regex for performance (~2x faster)
    # The checks below cumulatively build the score, replicating the original switch -Regex behavior.

    # 'Test|Tests' — "Test" substring covers "Tests"
    if ($Path.IndexOf("Test", [System.StringComparison]::OrdinalIgnoreCase) -ge 0) {
        $score += 50
    }

    # 'Service|Controller|Manager|Repository|Repo' — "Repo" covers "Repository"
    ...
    # '\.system\.txt$'
    if ($Path.EndsWith(".system.txt", [System.StringComparison]::OrdinalIgnoreCase)) {
        $score += 10
    }
